...
class AdminHomeController extends GetxController {
  ...
  Future<bool> updateSlotsStatus(String database, String id, int status) async {
    try {
  ...
  getAllAppointment() {
    ...
    try {
      var query =
          "select * from AppointmentModel where doctorid='${StaticData.doctorModel!.id}'";
  ...
  Future<String?> getSchedule() async {
    String id1 =
        StaticData.doctorModel!.id.replaceAll(RegExp(r'[^a-zA-Z]'), '');
    var query =
        "select count(isAvailable) as data from ${id1} where isAvailable=1";
    SQLQuery.getdata(query).then((value) {
      try {
        if (value != "" && value != null) {
